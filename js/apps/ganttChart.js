...
export function openGanttChart() {
    ...
    const appState = {
            ...
            this.flatTaskOrder.forEach((taskId, index) => {
                ...
                if (task.type === 'milestone') {
                    ...
                    switch(this.timeline.viewMode) {
                        ...
                        case 'month': width = (durationInDays / 30.4) * this.timeline.unitWidth; break;
        ...
        markDirty: function() {
            if (this.fileId) {
                window.windowManager.markWindowDirty(this.winId, true);
            }
        },
        markClean: function() {
            window.windowManager.markWindowDirty(this.winId, false);
        },
        saveFile: function() {
            ...
            if (!this.fileId) {
                const fileName = prompt("Nome do arquivo:", "roadmap.gantt");
                if (!fileName) return;
                
                this.fileId = fileManager.createFile({
                    name: fileName,
                    type: 'gantt-chart',
                    content: this.getData()
                });
                
                window.windowManager.updateWindowTitle(this.winId, fileName);
            } else {
                fileManager.updateFile(this.fileId, this.getData());
            }
            
            this.markClean();
            showNotification("Arquivo salvo com sucesso!", 2000);
        }
    };
