...
export function openGanttChart() {
    ...
    const content = `
        <style>
            ...
            .task-cell span.read-only-field { padding: 5px; color: var(--secondary-text-color); }
            ...
            .gantt-dependency-handle.start { left: -6px; }
            ...
            .gantt-context-menu-item { padding: 8px 12px; cursor: pointer; border-radius: 4px; }
            .gantt-context-menu-item:hover { background-color: var(--hover-highlight-color); }
            .gantt-context-menu-item.danger:hover { background-color: var(--danger-color); color: white; }
    ...
    const appState = {
        ...
        getData: function() { return JSON.parse(JSON.stringify(this.tasks)); }, // Retorna uma cópia limpa
        loadData: function(dataString, fileMeta) { 
            try { 
                const data = JSON.parse(dataString); 
                this.tasks = Array.isArray(data) ? data : [];
                this.tasks.forEach(t => { // Garante que tarefas antigas tenham campos novos
        ...
        pushState: function() {
            ...
            this.state.redoStack = []; // Limpa o redo stack ao fazer uma nova ação
            if (this.state.undoStack.length > 50) this.state.undoStack.shift(); // Limita o histórico
            this.updateUndoRedoButtons();
            this.markDirty();
        },
        undo: function() {
        ...
        renderEmptyState: function() {
            if (this.tasks.length === 0 && this.ui.sidebarBody) {
                this.ui.sidebarBody.innerHTML = `<div class="gantt-empty-state">
        ...
        updateAllParentTasks: function() {
            ...
            const processNode = (task) => {
                ...
                if (childTasks.length > 0) {
                    ...
                    const totalProgress = childTasks.reduce((sum, c) => sum + (c.progress || 0), 0);
        ...
        renderSidebar: function() {
            ...
            const renderTaskNode = (task, level) => {
                ...
                row.innerHTML = `
                    ...
                    <div class="task-cell">${isParent ? `<span class="read-only-field">${new Date(task.start+'T00:00:00').toLocaleDateString()}</span>` : `<input type="date" value="${task.start}" data-field="start">`}</div>
                    <div class="task-cell">${isParent || task.type === 'milestone' ? `<span class="read-only-field">${new Date(task.end+'T00:00:00').toLocaleDateString()}</span>` : `<input type="date" value="${task.end}" data-field="end">`}</div>
                `;
        ...
        updateCriticalPath: function() {
            // Remove a classe de todos os elementos primeiro
            ...
            // Lógica do CPM (Forward e Backward pass)
        ...
        handleSidebarClick: function(e) {
            ...
            this.state.selectedTaskId = row.dataset.taskId;
        ...
        handleBarInteraction: function(e) {
            ...
            const onMouseMove = (moveE) => {
                ...
                if (handle === 'left') {
                    ...
                    if (newStart <= initialEnd) task.start = this.dateUtils.format(newStart);
                } else if (handle === 'right') {
                    const newEnd = this.dateUtils.addDays(initialEnd, deltaDays);
                    if (newEnd >= this.dateUtils.parse(task.start)) task.end = this.dateUtils.format(newEnd);
                } else { // Mover barra inteira
                    task.start = this.dateUtils.format(this.dateUtils.addDays(initialStart, deltaDays));
                    task.end = this.dateUtils.format(this.dateUtils.addDays(initialEnd, deltaDays));
                }
                this.renderAll();
            };
            ...
            const onMouseUp = () => {
                ...
                if (moved) this.pushState(); // Salva estado apenas se houve movimento
            };
        ...
        showTooltip: function(e, task) {
            ...
            this.ui.tooltipEl.innerHTML = `
                ...
                <div class="gantt-tooltip-row"><span class="gantt-tooltip-label">Início:</span><span>${new Date(task.start+'T00:00:00').toLocaleDateString()}</span></div>
                <div class="gantt-tooltip-row"><span class="gantt-tooltip-label">Fim:</span><span>${new Date(task.end+'T00:00:00').toLocaleDateString()}</span></div>
        ...
        handleSidebarContextMenu: function(e) {
            ...
            this.ui.contextMenuEl.innerHTML = `
                ${task.type !== 'milestone' ? `<div class="gantt-context-menu-item" data-action="add-subtask"><i class="fas fa-plus"></i> Adicionar Sub-tarefa</div>` : ''}
                <div class="gantt-context-menu-item" data-action="delete"><i class="fas fa-trash"></i> Excluir Tarefa</div>
            ...
            this.ui.contextMenuEl.onclick = (menuE) => {
                ...
                if (actionTarget) {
                    ...
                    this.pushState();
                    task.color = colorTarget.dataset.color;
                    this.renderAll();
                }
        ...
        cleanup: () => {
             // Limpar event listeners globais, se houver
        }
    };
