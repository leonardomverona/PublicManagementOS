...
class WindowManager {
 ...
 salvas nesta janela. Deseja fechar mesmo assim e descartar as alterações?')) {
                    return;
                }
            }
        ...
        if (this.windows.has(winId)) {
            ...
            if (winData.maximized) {
                ...
                winData.originalRect = {
                    width: winEl.style.width, height: winEl.style.height,        this.appLaunchActions = {};
    }

    /**
     * Lógica detalhada e corrigida para ocultar/mostrar o Dock.
     */
    ...
    createWindow(title, content, options = {}) {
        ...
        if (options.appType) {
            windowContent.classList.add(`${options.appType.split(' ')[0]
    ...
    _addDragAndResize(win, titleBar, resizeHandle, winId) {
        ...
        window-control') || e.target.closest('.window-control')) return;
            const winData = this.windows.get(winId);
            ...
            document.onmouseup = () => {
                ...
                win.classList.removewin.appendChild(windowContent);
        ...
        const winData = {
            ...
            getScreenshot: async () => {
                try {
                    ...
                    if(controlsEl) controlsEl.('dragging');
            };
        };
        ...
        resizeHandle.onmousedown = (e) => {
            e.preventDefault();
            const winData = this.windows.get(winId);
            if (winData.maximized || (this.stageManager.isActive && this.stageManager.focusedAppId === winIdstyle.visibility = 'hidden';
            ...
            win.classList.                        windowWidth: win.scrollWidth, windowHeight: win.scrollHeight,
                        foreignObjectRendering: true, removeContainer: true,
                        backgroundColor: null
                    });
                     if(controlsEl) controlsEl.styleadd('resizing');

            document.onmousemove = (moveEvent) => {
                const newWidth =.visibility = 'visible';
                     if(resizeHandleEl) resizeHandleEl.style.visibility = 'visible initialWidth + (moveEvent.clientX - initialMouseX);
                const newHeight = initialHeight + (move';
                    return canvas.toDataURL('image/png');
                } catch (err) {
                    Event.clientY - initialMouseY);
                win.style.width = `${Math.max(parseInt(getconsole.warn("Erro ao gerar screenshot da janela:", err, winData.title);
                    const appIconActionComputedStyle(win).minWidth), newWidth)}px`;
                win.style.height = `${Math.Key = `open-${winData.appType.replace(/([A-Z])/g, '-$1').toLowerCase().split(' ')[0]}`;
                    const appIconEl = document.querySelector(`.dock-item[datamax(parseInt(getComputedStyle(win).minHeight), newHeight)}px`;
            };
            document.onmouseup = () => {
                document.onmousemove = document.onmouseup = null;
                document-action="${appIconActionKey}"]`);
                    const appIconText = appIconEl ? appIconEl..body.classList.remove('resizing');
                win.classList.remove('resizing');
                iftextContent : '⚙️';
                    const bgColor = getComputedStyle(document.documentElement).getPropertyValue('--button-bg').trim(this.stageManager.isActive) this.stageManager.updateStageThumbnails();
            };
        };
    }
    ...
    _createTaskbarItem(winId, title, appType) {
        const item();
                    ...
                    return `data:image/svg+xml;charset=UTF-8,<svg xmlns="http://www = document.createElement('div');
        item.className = 'taskbar-item';
        item.id = `taskbar-item-${winId}`;
        let actionKeyBase = appType ? appType.replace(/([.w3.org/2000/svg" width="100" height="75" viewBox="0 0 100 75"><rect width="100" height="75" fill="${bgColor}"/><text x="50" y="45" font-size="30"A-Z])/g, '-$1').toLowerCase().split(' ')[0] : 'generic';
        let actionKey = `open-${actionKeyBase}`;
        if (!this.appLaunchActions[actionKey] && this.app text-anchor="middle" dominant-baseline="middle" fill="${fgColor}">${appIconText || '🖼️'}</text><text x="50" y="65" font-size="10" text-anchor="middleLaunchActions[`new-${actionKeyBase}`]) {
             actionKey = `new-${actionKeyBase}`;
        } else if (!this.appLaunchActions[actionKey]) {
            const allActionKeys = Object." dominant-baseline="middle" fill="${fgColor}">${winData.title.substring(0,12)}</text></svg>`;
                }
            }
        };
        this.windows.set(winId,keys(this.appLaunchActions);
            const matchingActionKey = allActionKeys.find(k => k.endsWith(actionKeyBase));
            if (matchingActionKey) actionKey = matchingActionKey;
            else actionKey winData);
        this.makeActive(winId);
        this._addDragAndResize(win, titleBar, resizeHandle, winId);
        this._createTaskbarItem(winId, title, options = null;
        }
        const appIconEl = actionKey ? document.querySelector(`.dock-item[data-.appType);

        win.addEventListener('mousedown', () => this.makeActive(winId));
        if(thisaction="${actionKey}"]`) : null;
        const appIconText = appIconEl ? appIconEl.textContent :.stageManager.isActive) this.stageManager.updateStageThumbnails();
        this.updateDockVisibility(); '⚙️';


        item.innerHTML = `<span class="taskbar-item-icon" style="font-size:
        return winId;
    }

    makeActive(winId) {
        if (this. 1.2em; margin-right: 5px;">${appIconText}</span> <span class="activeWindowId && this.windows.has(this.activeWindowId)) {
            this.windows.get(this.taskbar-item-title">${title}</span>`;
        item.title = title;

        item.onclick = ()activeWindowId).element.classList.remove('active');
        }
        if (this.windows.has => {
            const winData = this.windows.get(winId);
            ...
            const winData = this.windows.get(winId);
            this.zIndex++;
.minimized) this.restoreWindow(winId);
            } else {
                if (winData.minimized) this.restoreWindow(winId);
                else if (this.activeWindowId === winId            winData.element.style.zIndex = this.zIndex;
            winData.element.classList.add('active');
            this.activeWindowId = winId;
            this._updateTaskbarActiveState() this.minimizeWindow(winId);
                else this.makeActive(winId);
            }
        };
        item.oncontextmenu = (e) => { e.preventDefault(); e.stopPropagation(); thiswinId);
            if (this.stageManager.isActive) {
                this.stageManager.focusApp(winId);
            }
        }
    ...
    closeWindow(winId) {
        ._showTaskbarItemContextMenu(e, winId, item); };
        this.taskbarItemsContainer.appendChild(item);
        this._updateTaskbarActiveState(winId);
    }
    
    _showif (this.windows.has(winId)) {
            const winData = this.windows.get(winId);
            if (winData.currentAppInstance && winData.currentAppInstance.isDirty) {
                if (!confirm('Você possui alterações não salvas nesta janela. Deseja fechar mesmo assim e descartar asTaskbarItemContextMenu(event, winId, taskbarItemElement) {
        const existingMenu = document.getElementById('taskbarContext');
        if (existingMenu) existingMenu.remove();

        const menu = document.createElement('div');
        menu.id = 'taskbarContext';
        menu.className = 'taskbar-item alterações?')) {
                    return;
                }
            }
            if (winData.currentAppInstance && typeof winData.currentAppInstance.cleanup === 'function') {
                 winData.currentAppInstance.-context-menu';
        ...
        cleanup();
            ...
            winData.element.remove();
            this.windows.delete(winId);
            this._removeTaskbarItem(winIdcloseButton.onclick = (e) => { e.stopPropagation(); this.closeWindow(winId); menu.remove(); };
        ...
        let left = itemRect.left + (itemRect);
            if (this.activeWindowId === winId) this.activeWindowId = null;
            if(this.stageManager.isActive) this.stageManager.updateStageThumbnails();
            this.updateDockVisibility();
        }
    ...
    minimizeWindow(winId) {
        if (this.windows.has(winId)) {
            const winData = this.windows.get(winId);
.width / 2) - (menuRect.width / 2);

        if (top < 0) top = itemRect.bottom + 5;
        if (left < 0) left = 5;
        if (left + menuRect.width > window.innerWidth) left = window.innerWidth - menuRect            const dock = document.getElementById('appDock');
            const dockRect = dock.getBoundingClientRect();
            const targetX = dockRect.left + (dockRect.width / 2);
            const targetY = dockRect.top + (dockRect.height / 2);
            winData.element.style.transformOrigin = `${targetX - winData.element.offsetLeft}px ${targetY - winData.element.offsetTop}px`;.width - 5;
    ...
    _updateTaskbarItemTitle(winId, newTitle) { const item = document.getElementById(`taskbar-item-${winId}`); if (item) {
            const winData = this.windows.get(winId);
            const winEl = winData.element;
            if (winData.maximized) {
                winEl.classList.remove('maximized');
                if (winData.originalRect) {
                    winEl.style.width = winData.originalRect.width;
                    winEl.style.height = winData.originalRect.height;
 { const titleEl = item.querySelector('.taskbar-item-title'); if(titleEl) titleEl.textContent = newTitle; item.title = newTitle; } }
    _updateTaskbarActiveState(winIdToActivate) { this.taskbarItemsContainer.querySelectorAll('.taskbar-item').forEach(item => item                    winEl.style.top = winData.originalRect.top;
                    winEl.style.left = winData.originalRect.left;
                }
                winData.maximized = false;
            } else {
                winData.originalRect = {
                    ...
                    top: winEl.style.top, left: winEl..classList.remove('active')); const activeItem = document.getElementById(`taskbar-item-${winIdToActivate}`); if (activeItem) activeItem.classList.add('active'); }
    _updateTaskbarItemVisual(winId, isMinimized) { const item = document.getElementById(`taskbar-item-${winId}`); if (item)style.left
                };
                winEl.classList.add('maximized');
                winData.maximized = true;
            }
    ...
    restoreWindow(winId { item.style.opacity = isMinimized ? '0.7' : '1'; } }
}

class ThemeManager {
    constructor(palettes) {
        this.isDark = localStorage.getItem(STORAGE_KEYS.THEME_DARK_MODE) === 'true';
        this.accentColor = localStorage.getItem(STORAGE_KEYS.THEME_ACCENT_COLOR) || (this.isDark ? 'var(--accent-dark)' : ') {
        if (this.windows.has(winId)) {
            const winData = this.windows.get(winId);
            winData.element.style.transformOrigin = 'bottom center';
            winData.element.classList.remove('minimized');
            winData.minimized = false;
            this.makeActive(winId);
            this._updateTaskbarItemVisual(winId, false);var(--accent-light)');
...
isDark;
        ...
        if (this.accentColor === (this.isDark ? 'var(--accent-light)' : 'var(--accent-dark)')) {
            this.setAccentColor(this.isDark ? 'var(--accent-dark)' : 'var(--accent             if(this.stageManager.isActive) this.stageManager.updateStageThumbnails();
        }
    }
    ...
    _addDragAndResize(win, titleBar, resizeHandle, winId) {
        ...
        titleBar.onmousedown = (e) => {
            ...
            const winData = this.windows.get(winId);
            if (winData.maximized || (this-light)');
        } else {
             this.applyThemeVariables();
        }
    ...
    setAccentColor(colorNameOrValue) {
        ...
        localStorage.setItem(STORAGE_KEYS.THE.stageManager.isActive && this.stageManager.focusedAppId === winId)) return;
            this.makeActive(winId);
            offsetX = e.clientX - win.offsetLeft;
            offsetY = e.clientY - win.offsetTop;
            document.body.classList.add('dragging');
            ...
            document.onmousemove = (moveEvent) => {
                win.style.left = `${moveEvent.clientX - offsetX}px`;
                win.style.top = `${moveEvent.ME_ACCENT_COLOR, colorNameOrValue);
        this.applyThemeVariables();
        this._updatePaletteSelection();
    }
    _setAccentColorVariableInternal(colorVarOrHex) {
        ...
        rootEl.style.setPropertyclientY - offsetY}px`;
            };
            document.onmouseup = () => {
                document.onmousemove = document.onmouseup = null;
                document.body.classList.remove('dragging');
        ...
        resizeHandle.onmousedown = (e) => {
            ...
            if (winData.maximized || (this.stageManager.isActive && this.stageManager.focusedAppId === winId)) return;
            this.makeActive(winId);
            const initialWidth = win('--accent-light', actualColorValue);
        ...
        if (actualColorValue.startsWith('#')) {
            ...
            document.documentElement.style.setProperty('--accent-light-translucent.offsetWidth;
            const initialHeight = win.offsetHeight;
            const initialMouseX = e.clientX;
            const initialMouseY = e.clientY;
            document.body.classList.add('resizing');
            ...
            document.onmousemove = (moveEvent) => {
                ...
                const newHeight = initial', `rgba(${r},${g},${b},0.1)`);
            document.documentElement.style.setProperty('--accent-dark-translucent', `rgba(${r},${g},${b},0.15)`);
             ...
             const match = actualColorValue.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)(Height + (moveEvent.clientY - initialMouseY);
            ...
            document.onmouseup = () => {
                document.onmousemove = document.onmouseup = null?:,\s*([\d.]+))?\)/);
             if(match) {
    ...
    _createTaskbarItem(winId, title, appType) {
        ...
        item.className = 'taskbar-item';
        item.id = `taskbar-item-${winId}`;
        let actionKeyBase = appType ?documentElement;
        const themePrefix = this.isDark ? 'dark' : 'light';
        const propsToUpdate = ['background', 'text-color', 'secondary-text-color', 'window-bg', 'toolbar-bg', 'button-bg', 'button-text-color', 'input-bg', 'input-border-color', 'separator-color', 'hover-highlight-color', 'current-window-bg-rgb', 'current-background- appType.replace(/([A-Z])/g, '-$1').toLowerCase().split(' ')[0] : 'generic';
        let actionKey = `open-${actionKeyBase}`;
        if (!this.appLaunchActions[actionKey] && this.appLaunchActions[`new-${actionKeyBase}`]) {
            ...
            const matchingActionKey = allActionrgb-for-stage'];
        propsToUpdate.forEach(prop => {
         ...
         this._Keys.find(k => k.endsWith(actionKeyBase));
            if (matchingActionKey) actionKey = matchingActionKey;
            else actionKey = null;
        }
        ...
        item.innerHTML = `<spanupdateUIColorDependents();
    }
        ...
        window.windowManager.windows.forEach(winData => {
            if (winData.appType === 'mindmap' && winData.jmInstance) {
                winData.jmInstance.set_theme(this.isDark ? 'dark' : 'primary');
                 ...
                 const fgColorPicker = winData.element.querySelector('input[id^="fgColor class="taskbar-item-icon" style="font-size: 1.2em; margin-right: 5px;">${appIconText}</span> <span class="taskbar-item-title">${title}</span>`;
        item.title = title;

        item.onclick = () => {
            const winData = this.windows.get(winId);
            if (this.stageManager.isActive) {
                this.stageManager.Picker_"]');
    ...
    _setupAccentPalette() {
        ...
        if (!paletteContainer || !darkModeTogglefocusApp(winId);
                if (winData.minimized) this.restoreWindow(winId);
            } else {
                if (winData.minimized) this.restoreWindow(winId);
        ...
        currentPalette.forEach(item => {
            ...
            colorOption.style.backgroundColor = item.color.startsWith('var(') ?
                getComputedStyle(document.documentElement) => { e.preventDefault(); e.stopPropagation(); this._showTaskbarItemContextMenu(e, winId, item); };
        this.taskbarItemsContainer.appendChild(item);
        this._updateTaskbarActiveState(winId);
    }
    ...
    _showTaskbarItemContextMenu(event, winId, taskbarItemElement) {
        ...
        const menu = document.createElement('div');
        menu.id = 'taskbarContext).getPropertyValue(item.color.replace('var(','').replace(')','')).trim() :
                item.color;
     ...
     _updatePaletteSelection() {
        ...
        paletteContainer.querySelectorAll('.color-option').document.body.appendChild(menu);
        const menuRect = menu.getBoundingClientRect();
        const itemRect = taskbarItemElement.getBoundingClientRect();
        let top = itemRect.top - menuRect.height - 5;
...
class StageManager {
    constructor(windowMgr) { this.windowManager = windowMgr; this.isActive = false; this.overlay = document.getElementById('stageManagerOverlay'); this.sidebar = document.getElementById('stageManagerSidebar'); this.mainArea = document.getElementById('stageManagerMainArea'); this.toggleButton = document.getElementById('stageManagerToggle'); this.focusedAppId = null; this.originalWindowStates = newmenu.style.top = `${top}px`;
        menu.style.left = `${left}px`;
        menu.style.display = 'block';
...
.height, zIndex: winData.element.style.zIndex, transform: winData.element.style.transform || '' }); } }); const activeOrFirstVisible = this.windowManager.activeWindowId || Array.from(this.windowManager.windows.keys()).find(id => !this.windowManager.windows.get(id).minimized && this.windowManager.windows.get(id).element.style.display !== 'none' ); if (activeOrFirstVisible) { this.focusApp(activeOrFirstVisible); } else { this.updateStageThumbnails(); } } else { this.restoreWindows(); this.originalWindowStates.clear(); } }
    async updateStageThumbnails() { if (!this.sidebar) return; this.sidebar.innerHTML = ''; const open    constructor(palettes) {
        this.isDark = localStorage.getItem(STORAGE_KEYS.THEME_DARK_MODE) === 'true';
...
const accentPalettes = { light: [{ name: "Azul Padrão", color: "var(--accent-light)" }, { name: "Rosa", color: "#FF2D55" }, { name: "Picker_"]');
    ...
    _setupAccentPalette() {
        ...
        Verde", color: "#34C759" }, { name: "Laranja", color: "#FF9500" }, { name: "Roxo", color: "#AF52DE" }, { name: "Amarelo", color: "#FFCC00" }, { name: "Grafite", color: "#8E8E93"}, { name: "Ciano", color: "#5AC8FA"}], dark: [{ name: "Azul Padrão", color: "var(--accent-dark)" }, { name: "Rosa", color: "#FF375F" }, { name: "Verde", color: "#30D158" }, { name: "Laranja", color: "#FF9F0A" }, { name: "Roxo", color: "#BF5AF2" }, { name: "AmarepaletteContainer.innerHTML = '';
        const currentPalette = this.isDark ? this.palettes.dark : this.palettes.light;
        currentPalette.forEach(item => {
            const colorOption = document.createElement('div');
            colorOption.className = 'color-option';
            colorOption.style.backgroundColor = item.color.startsWith('var(') ?
                ...
                item.color;
            colorOption.title = item.name;
            colorOption.dataset.colorValue = item.lo", color: "#FFD60A" }, { name: "Grafite", color: "#98989D"}, { name: "Ciano", color: "#64D2FF"} ] };
...
export function initializeWebOS() {
    ...
    window.windowManager.appLaunchActions = { 
     ...
     _updatePaletteSelection() {
        const paletteContainer = document.getElementById('accentColorPaletteMenu');
        if (!paletteContainer) return;
        paletteContainer.querySelectorAll('.color-option').': apps.openProjectTasks, 
        ...
        'open-5w2h-tool': appsforEach(opt => opt.classList.remove('selected'));
        const selectedOption = paletteContainer.querySelector(`.color-option[data-color-value="${this.accentColor}"]`);
        if (selectedOption) selectedOption.classList.add('selected');
     }
}
...
class StageManager {
    ...
    toggle() { this.isActive = !this.isActive; this.overlay.classList.toggle('active', this.open5W2HTool 
    };
    ...
    document.onclick = (e) => { const dm = document.getElementById('desktopContextMenu'); if (dm && dm.style.display === 'block' && !dm.contains(e.target)) { hideDesktopContextMenu();.isActive); if(this.toggleButton) this.toggleButton.classList.toggle('active', this.isActive); if (this.isActive) { this.windowManager.windows.forEach((winData, winId) => { if (!winData.minimized && winData.element.style.display !== 'none') { this.originalWindowStates.set(winId, { left: winData.element.style.left, top: winData.element.style.top, width: winData.element.style.width, height: winData.element.style.height, zIndex: winData.element.style.zIndex, transform: winData.element.style.transform || '' }); } }); const activeOrFirstVisible = this.windowManager.activeWindowId || Array.from(this.windowManager.windows.keys()).find(id => !this.windowManager.windows.get(id).minimized && this.windowManager.windows.get(id).element.style.display !== 'none' ); if } };
        ...
        wallpaperInput.addEventListener('change', e => {
            if (!e.target.files || !e.target.files[0] || !e.target.files[0].type.startsWith('image/')) {
                if (e.target.files && e.target.files[0]) {
                    showNotification (activeOrFirstVisible) { this.focusApp(activeOrFirstVisible); } else { this.updateStageThumbnails(); } } else { this.restoreWindows(); this.originalWindowStates.clear(); } }
    async updateStageThumbnails() { if (!this.sidebar) return; this.sidebar.innerHTML = ''; const openWindows = []; this.windowManager.windows.forEach(winData => { if (!winData.minimized && winData.element.style.display !== 'none') { openWindows.push(winData); } }); for (const winData of openWindows) { if (winData.element.id === this.focusedAppId && this.isActive) continue; const thumb = document.createElement('div'); thumb.className = 'stage-app-thumbnail'; thumb.dataset("Por favor, selecione um arquivo de imagem válido.", 3000);
                }
                return;
            }
    ...
    const isTouchDevice = 'ontouchstart.mainArea.getBoundingClientRect(); const sidebarWidth = this.sidebar.offsetWidth + 20; const availableWidth = mainAreaRect.width - (this.isActive ? sidebarWidth : 0); let targetWidth = Math.min(parseFloat(this.originalWindowStates.get(id)?.width || winEl.offsetWidth), availableWidth * 0.75); let targetHeight = (targetWidth / parseFloat(this.originalWindowStates.get(id)?.width || winEl.offsetWidth)) * parseFloat(this.originalWindowStates.get(id)?.height || winEl.offsetHeight); if (targetHeight > mainAreaRect.height * 0.85) { targetHeight = mainAreaRect.' in window || navigator.maxTouchPoints > 0;
    ...
    showNotification(`Bem-vindo(aheight * 0.85; targetWidth = (targetHeight / parseFloat(this.originalWindowStates.get(id)?.height || winEl.offsetHeight)) * parseFloat(this.originalWindowStates.get(id)?.width || winEl.offsetWidth); } winEl.style.width = `${targetWidth}px`; winEl.style.height = `${targetHeight}px`; winEl.style.left = `${(this.isActive ? sidebarWidth : 0) de volta!`, 3500);
}
...
function chooseWallpaper() { 
    const wallpaperInput = document.getElementById('wallpaperInput');
    if (wallpaperInput) {
) + (availableWidth - targetWidth) / 2}px`; winEl.style.top = `${(mainAreaRect.height - targetHeight) / 2}px`; winEl.style.transform = 'scale(1)'; } else { winEl.style.display = 'none'; } }); this.updateStageThumbnails(); }
    restoreWindows() { this.windowManager.windows.forEach((winData, winId) => { const originalState =        wallpaperInput.click();
    }
}
function showDesktop() { window.windowManager.windows.forEach((wData, winId) => { if (!wData.minimized) window.windowManager.minimizeWindow(win this.originalWindowStates.get(winId); if (originalState) { winData.element.style.left = originalState.left; winData.element.style.top = originalState.top; winData.element.style.width = originalState.width; winData.element.style.height = originalState.heightId); }); }
function shutdown() {
    let hasUnsavedChanges = false;
    window.windowManager.windows.forEach(winData => { if (winData.currentAppInstance?.isDirty) hasUnsavedChanges = true; });
    let confirmMsg = 'Deseja realmente sair da sua conta?';
    if (hasUnsavedChanges) {
        confirmMsg = 'Você possui alterações não salvas que serão perdidas. Deseja mesmo sair?';; winData.element.style.zIndex = originalState.zIndex; winData.element.style.transform = originalState.transform; } if (!winData.minimized) winData.element.style.display = '
    }
...
function setupDesktopContextMenuListeners() {
    ...
    const colorPaletteContainer = document.getElementById('contextMenuColorPaletteflex'; }); this.focusedAppId = null; if(window.windowManager.activeWindowId) window.windowManager.makeActive(window.windowManager.activeWindowId); this.updateStageThumbnails(); }
}
    
// --- INICIALIZAÇÃO E EVENTOS GLOBAIS ---

const accentPalettes = { light: [{ name: "Azul Padrão", color: "var(--accent-light)" }, { name: "Rosa", color: "#FF2D55" }, { name: "Verde", color: "#34Container'); 
    ...
    deskMenu.querySelectorAll('div.context-menu-item[data-action]').forEach(item => {
        item.onclick = (e) => {
            e.stopPropagation(); 
            C759" }, { name: "Laranja", color: "#FF9500" }, { name: "Roxo", color: "#AF52DE" }, { name: "Amarelo", color: "#FFCC00" }, { name: "Grafite", color: "#8E8E93"}, { name: "Ciano", color: "#5AC8FA"}], dark: [{ name: "Azconst action = item.dataset.action;
            if (action === 'toggle-theme-settings') {
                ...
                if (!isDisplayed) window.themeul Padrão", color: "var(--accent-dark)" }, { name: "Rosa", color: "#FF375F" }, { name: "Verde", color: "#30D158"Manager._setupAccentPalette(); 
                return;
            }
            const globalActions = { 'open-file-system': apps.openFileSystem, 'choose-wallpaper': chooseWallpaper, 'show-desktop': showDesktop, ' }, { name: "Laranja", color: "#FF9F0A" }, { name: "Roxoshutdown': shutdown };
...
", color: "#BF5AF2" }, { name: "Amarelo", color: "#FFD60A" }, { name: "Grafite", color: "#98989D"}, { name:}

function showDesktopContextMenu(x, y) {
    const deskMenu = document.getElementById('desktopContextMenu "Ciano", color: "#64D2FF"} ] };

export function initializeWebOS() {
'); if (!deskMenu) return; if (!desktopMenuInitialized) setupDesktopContextMenuListeners();
    window.themeManager._    if (window.webOSInitialized) return;
    window.webOSInitialized = true;

    window.windowManager = new WindowManager();
    window.themeManager = new ThemeManager(accentPalettes);
setupAccentPalette(); const colorPaletteContainer = document.getElementById('contextMenuColorPaletteContainer'); if (colorPaletteContainer    
    updateClockTime();
    setInterval(updateClockTime, 10000);
    ...
    deskMenu.style.left = `${x}px`; deskMenu.style.top = `${y}px`; deskMenu.style.display = 'block';window.windowManager.windows.forEach(winData => { if (winData.currentAppInstance?.isDirty const menuRect = deskMenu.getBoundingClientRect(); const taskbarHeight = parseInt(getComputedStyle(document.documentElement).getPropertyValue) hasUnsavedChanges = true; });
        if (hasUnsavedChanges) { e.preventDefault(); e('--taskbar-height'), 10) || 0;
    if (menuRect.right > window.returnValue = ''; }
    });

    window.windowManager.appLaunchActions = { 
        '.innerWidth) deskMenu.style.left = `${Math.max(0, x - menuRect.width)}open-file-system': apps.openFileSystem, 
        'open-gantt-chart': apps.openGanttChart, 
        'open-project-tasks': apps.openProjectTasks, px`;
    if (menuRect.bottom > (window.innerHeight - taskbarHeight)) deskMenu.style
        'open-kanban-board': apps.openKanbanBoard, 
        'open-sw.top = `${Math.max(0, y - menuRect.height)}px`;
    deskMenu.style.zIndex = (window.windowManager.zIndex || 100) + 5000;ot-analysis': apps.openSWOTAnalysis, 
        'open-sipoc-matrix': apps.openSIPOCMatrix, 
        'open-okr-tracker': apps.openOKRTracker, 

}
function hideDesktopContextMenu() { const dm = document.getElementById('desktopContextMenu'); if(dm) dm.style.display = 'none'; }
let longPressTimer, touchStartX, touchStartY; const longPress        'open-ishikawa-diagram': apps.openIshikawaDiagram, 
        'open-bpmnDuration = 700;
document.getElementById('desktop').addEventListener('touchstart', (e) => { if-modeler': apps.openBPMNModeler, 
        'open-mind-map': apps (e.touches.length > 1) return; touchStartX = e.touches[0].clientX; touchStartY.openMindMap, 
        'open-contract-manager': apps.openContractManager, 
        'open-checklist-tool': apps.openChecklistTool, 
        'open-ncr-tool = e.touches[0].clientY; longPressTimer = setTimeout(() => { if (longPressTimer) { e.preventDefault(); showDesktopContextMenu(touchStartX, touchStartY); } }, longPressDuration); }, {passive: false});': apps.openNCRTool, 
        'open-pdca-tool': apps.openPDCAT
function clearLongPressTimer() { clearTimeout(longPressTimer); longPressTimer = null; }
document.getElementById('ool, 
        'open-5w2h-tool': apps.open5W2HTool 
    };
    
    document.querySelectorAll('#appDock .dock-item[data-action]').forEach(desktop').addEventListener('touchend', clearLongPressTimer); document.getElementById('desktop').addEventListener('touchcancel', clearLongPressTimer);
document.getElementById('desktop').addEventListener('touchmove', (e) => { if (longitem => {
        const action = item.dataset.action;
        if (window.windowManager.appPressTimer && touchStartX && touchStartY && (Math.abs(e.touches[0].clientX-touchStartXLaunchActions[action]) {
            item.onclick = (e) => { e.stopPropagation(); window.windowManager.appLaunchActions[action](); };
        }
    });
    
    const desktopEl = document.) > 10 || Math.abs(e.touches[0].clientY-touchStartY) > 10)) clearLongPressTimer(); });
